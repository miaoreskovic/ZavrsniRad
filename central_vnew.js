      ...
      services.forEach(function(service) {
        ...
        service.discoverCharacteristics([], function(err, characteristics) {
          ...
          characteristics.forEach(function(characteristic) {
            ...
            if (pizzaBakeCharacteristicUuid == characteristic.uuid) {
...
function bakePizza() {
		...
		pizzaBakeCharacteristic.on('read', function(data, isNotification) {
            ...
            if (data.length === 1) {
                ...
                result == pizza.PizzaBakeResult.SITTING? 'sitting' :
                result == pizza.PizzaBakeResult.LAYING ? 'laying' :
        ...
        if (!err) {
          ...
          pizzaBakeCharacteristic.on('read', function(data, isNotification) {
            ...
            if (data.length === 1) {
              var result = data.readUInt8(0);
              console.log('Animal is',
                result == pizza.PizzaBakeResult.HALF_BAKED ? 'sitting' :
                result == pizza.PizzaBakeResult.BAKED ? 'laying' :
                result == pizza.PizzaBakeResult.CRISPY ? 'crispy.' :
                result == pizza.PizzaBakeResult.BURNT ? 'burnt.' :
                result == pizza.PizzaBakeResult.ON_FIRE ? 'on fire!' :
                  'unknown?');
